...
The third case is the hardest as we need to perform some preprocessing. We need to consider the maximum sums from both the left-side (prefix) and right-side (suffix) of the middle element. To do so, we need to pre-compute the local maximum for the prefix and suffix for every position, which is similar to how the `dp` array is defined in the dynamic programming approach. We add the local maximum sums from both prefix and suffix of the middle element, and minus the middle element value as it is double-counted.
